...
def build_window(app, root):
    ...
    width = max(860, min(1000, root.winfo_screenwidth() - 64))
    height = max(620, min(760, root.winfo_screenheight() - 100))
    root.geometry(f'{width}x{height}+{max(0, (root.winfo_screenwidth() - width) // 2)}+32')
