...
class PositionTests(unittest.TestCase):
    def test_caret_edges_and_monitors(self):
        area = (0, 0, 1920, 1040)
        self.assertEqual(indicator_position((400, 300, 1, 20), area, 56, 38), (409, 328))
        self.assertEqual(indicator_position((1900, 1020, 1, 20), area, 56, 38), (1836, 974))
        self.assertEqual(indicator_position((-1300, -100, 1, 20),
                         (-1920, -200, 1920, 1080), 56, 38), (-1291, -72))
        self.assertEqual(indicator_position((-4000, -4000, 1, 20), area, 56, 38), (8, 8))
        # 200% scale: both indicator and gap remain beside the physical caret.
        self.assertEqual(indicator_position((800, 600, 2, 40),
                         (0, 0, 3840, 2080), 112, 76, 16), (818, 656))
