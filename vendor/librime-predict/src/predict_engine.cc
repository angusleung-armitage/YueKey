...
an<PredictEngine> PredictEngineComponent::GetInstance(const Ticket& ticket) {
  if (ticket.schema && ticket.engine) {
    for (auto it = predict_engine_by_input_context_.begin();
         it != predict_engine_by_input_context_.end();) {
      if (it->second.expired())
        it = predict_engine_by_input_context_.erase(it);
      else
        ++it;
    }
    auto found = predict_engine_by_input_context_.find(ticket.engine);
    if (found != predict_engine_by_input_context_.end()) {
    ...
    if (new_instance) {
      predict_engine_by_input_context_[ticket.engine] = new_instance;
      return new_instance;
    }
  }
  return nullptr;
