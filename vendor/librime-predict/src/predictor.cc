...
Predictor::Predictor(const Ticket& ticket, an<PredictEngine> predict_engine)
  ...
  commit_connection_ = context->commit_notifier().connect(
      [this](Context*) { commit_pending_ = true; });
...
void Predictor::OnContextUpdate(Context* ctx) {
  ...
  // Context::Commit notifies first, then clears composition. A plain Clear
  // (frontend reset/focus cancellation) must not reuse the last commit history.
  const bool fresh_commit = commit_pending_;
  commit_pending_ = false;
  if (!fresh_commit || !ctx->get_option("prediction") || last_action_ == kDelete) {
    predict_engine_->Clear();
    iteration_counter_ = 0;
    return;
  }
